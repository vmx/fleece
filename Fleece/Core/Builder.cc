...
namespace fleece::impl::builder {
    ...
    /// The class that implements the Builder API.
    class Builder {
    public:
        ...
        RetainedConst<Value> buildValue() {
            switch (peekToken()) {
                case '[': {
                    ...
                    _buildIntoArray(array, nullptr);
                    finished();
                    return array.get();
                }
                case '{': {
                    Retained<MutableDict> dict = MutableDict::newDict();
                    _buildIntoDict(dict, nullptr);
                    finished();
                    return dict.get();
                }
                default:
        ...
        // Adds to an existing mutable Dict.
        ...
        // Adds to an existing mutable Array.
        ...
        // Writes to an Encoder.
        void buildInto(Encoder &enc) {
            _buildValue(enc);
            finished();
        }
    ...
    private:
        ...
        // Abstraction of "where to put the parsed value", either a slot or an encoder.
        struct Destination {
            ValueSlot* const slot;
            Encoder* const   enc;
            slice const      key;

            Destination(ValueSlot &slot)         :slot(&slot),   enc(nullptr) { }
            Destination(Encoder &enc)            :slot(nullptr), enc(&enc) { }
            Destination(Encoder &enc, slice key) :slot(nullptr), enc(&enc), key(key) { }
...
#if __APPLE__
            void setCFValue(CFTypeRef value) const {
                if (slot) {
                    FLSlot_SetCFValue((FLSlot)slot, value);
                } else {
                    if (key)
                        enc->writeKey(key);
                    enc->writeCF(value);
        ...
        const bool _buildValue(const Destination &dst) {
            switch (peekToken()) {
                case '[':
                    if (dst.slot) {
                        Retained<MutableArray> marray = MutableArray::newArray();
                        _buildIntoArray(marray, nullptr);
                        dst.slot->set(marray);
                    } else {
                        dst.enc->beginArray();
                        _buildIntoArray(nullptr, dst.enc);
                        dst.enc->endArray();
                    }
                    break;
                case '{':
                    if (dst.slot) {
                        Retained<MutableDict> mdict = MutableDict::newDict();
                        _buildIntoDict(mdict, nullptr);
                        dst.slot->set(mdict);
                    } else {
                        dst.enc->beginDictionary();
                        _buildIntoDict(nullptr, dst.enc);
                        dst.enc->endDictionary();
                    }
                    break;
                case 'n':
                    readIdentifier("null");
                ...
                case '5': case '6': case '7': case '8': case '9':
                    readLiteralNumber(dst);
                    break;
                case '"':
                case '\'':
                    dst.set(readLiteralString());
                    break;
                case '%':
                    get();
                    return putParameter(dst);
                default:
        ...
        // Parses JSON5 array elements, adding to a mutable array or encoder, until `]`.
        void _buildIntoArray(MutableArray *marray, Encoder *enc) {
            get();      // skip the opening '[' *without verifying*
            while (peekToken() != ']') {
                if (marray) {
                    if (!_buildValue(marray->appending()))
                        marray->remove(marray->count() - 1, 1);
                } else {
                    _buildValue(Destination(*enc));
                }
        ...
        // Parses JSON5 dictionary elements, adding to a mutable dict or encoder, until `}`.
        void _buildIntoDict(MutableDict *mdict, Encoder *enc) {
            get();      // skip the opening '{' *without verifying*
            char c;
            while ('}' != (c = peekToken())) {
                // Scan key:
                ...
                // Value:
                if (mdict) {
                    if (!_buildValue(mdict->setting(key)))
                        mdict->remove(key);
                } else {
                    _buildValue(Destination(*enc, key));
                }
...
#pragma mark - PARAMETER SUBSTITUTION:
        ...
        // @return  True if a value was added, false if not (because it skipped a default value.)
        bool putParameter(const Destination &dst) {
            char c = get();
            ...
            switch (c) {
                ...
                case '@': {
                    ...
                    CFTypeRef param = va_arg(_args, CFTypeRef);
                    if (!param)
                        return false;
                    dst.setCFValue(param);
                    return true;
                }
#endif
                default:
...
#pragma mark - LITERALS:
        ...
        // Reads a numeric literal, storing it in the Destination.
        void readLiteralNumber(const Destination &dst) {
...
#pragma mark - JSON(ish) LEXER:


        // Reads alphanumeric characters, returning the identifier as a string.
        // (The 1st char is accepted without checking, on the assumption the caller already
...
#pragma mark - PUBLIC API:
    ...
    void Update(MutableArray *array, const char *format, ...) {
        va_list args;
        va_start(args, format);
    ...
    void Update(MutableDict *dict, const char *format, ...) {
        va_list args;
        va_start(args, format);
    ...
    void VUpdate(Value *v, const char *format, va_list args) {
...
namespace fleece::impl {
    ...
    void Encoder::writef(const char *format, ...) {
        va_list args;
        va_start(args, format);
        builder::Builder(format, args).buildInto(*this);
        va_end(args);
    }

    void Encoder::vwritef(const char *format, va_list args) {
        builder::Builder(format, args).buildInto(*this);
    }
