...
namespace fleece::impl::builder {
    ...
    /** Creates a MutableArray or MutableDict by reading the format string and following arguments.
        ...
        If a string/value specifier is given a NULL pointer, nothing is written.
    ...
    void Update(MutableArray*, const char *format, ...) __printflike(2, 3);
    ...
    void Update(MutableDict*, const char *format, ...) __printflike(2, 3);

    /** Variant of \ref Update that takes a pre-existing `va_list`. */
    void VUpdate(Value*, const char *format, va_list args);
