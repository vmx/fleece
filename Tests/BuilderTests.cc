...
TEST_CASE("Builder Encode Array", "[Builder]") {
    Encoder enc;
    enc.writef("[%s, %d, %f]",
               "Zegpold", 12, 3.14);
    auto doc = enc.finishDoc();
    CHECK(doc->root()->toJSONString() == R"(["Zegpold",12,3.14])");
}


TEST_CASE("Builder Encode Dict", "[Builder]") {
    Encoder enc;
    enc.writef("{name:%s, size:%d, weight:%f}",
               "Zegpold", 12, 3.14);
    auto doc = enc.finishDoc();
    CHECK(doc->root()->toJSONString() == R"({"name":"Zegpold","size":12,"weight":3.14})");
}


TEST_CASE("Builder Encode Dict Ignored Params", "[Builder]") {
    Encoder enc;
    const char *name = nullptr;
    enc.writef("{name:%s, size:%-d, weight:%f}",
               name, 0, 3.14);
    auto doc = enc.finishDoc();
    CHECK(doc->root()->toJSONString() == R"({"weight":3.14})");
}
